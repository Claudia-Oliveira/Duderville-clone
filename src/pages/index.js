    ...
    render() {
        ...
        return (
            <div className={styles.page}>
                ...
                <div className={styles.home__cases}>
                    <h2 className={styles.home__pages_title}>{t('home:cases__title')}</h2>

                    <ul className={`${styles.list} ${styles.home__links} `}>
                        {t('home:navigation', { returnObjects: true }).map((item) => {
                            return (
                                <li key={item.button.copy} className={`${styles.item} ${styles.home__button_li}`}>
                                    <Button href={item.button.href} className={styles.home__button_cases}>
                                        <span className={styles.home__button_order}>{item.button.number}</span>
                                        <span className={styles.home__button_label}>{item.button.copy}</span>
                                        <svg className={styles.home__button_icon} viewBox="0 0 16 5">
                                            <path
                                                fillRule="evenodd"
                        ...
                        {/* <li className={`${styles.item} ${styles.home__button_li}`}>
                            <Button href="/the-work" className={styles.home__button_cases_work}>
                                <span className={styles.home__button_order}>{t('home:button__more_work_number')}</span>
                                <span className={styles.home__button_label}>{t('home:button__more_work')}</span>
                                <svg className={styles.home__button_icon_work} viewBox="0 0 16 5">
                                    <path
                                        fillRule="evenodd"
                                        fill="rgb(48, 48, 48)"
                                        d="M0.391,2.35 L10.945,1.976 C10.945,1.196 10.942,0.747 10.914,0.53 C12.586,0.936 14.47,1.696 15.739,2.456 C14.83,3.270 12.634,4.12 10.914,4.859 C10.998,4.105 10.945,3.765 10.945,2.936 L0.391,2.821 L0.391,2.35 Z"
